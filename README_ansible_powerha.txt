...
7.	Tarball Extraction: Extract the Ansible filesets from the provided Tarball from PowerHA installables. The tarball is located at the centralized node at path /usr/es/sbin/cluster/samples/ansible/. To extract the files, follow these steps:
	...
	5.	Extract the contents of the tarball by running the below command.
		...
		After the extraction, you will find two new folders with the following contents:
			/.ansible/collections/ansible_collections/ibm/power_ha: Contains Ansible roles, playbooks, and a template file.
			/etc/ansible: Contains the hosts file, ansible.cfg file, a template file, hosts file and an external variable file.
...
2.	Update Ansible External Variables: Open the external_vars.yml file located in the /etc/ansible directory. Update the file with the required NODE_DETAILS and NODES and test case details specific to your environment. This file allows you to customize variables that will be used in your Ansible playbooks.
...
3.	Execute Ansible Playbooks: Once above steps are properly configured and the necessary variables updated, you can now execute Ansible playbooks. 
	Go to the path /.ansible/collections/ansible_collections/ibm/power_ha and run the Ansible commands along with the desired tags. 
	...
	# cd /.ansible/collections/ansible_collections/ibm/power_ha/playbooks 
        # ansible-playbook demo_playbook.yml --tags <tag1>,<tag2>
...
You can use the following playbooks to install/uninstall or configure/unconfigure PowerHA SystemMirror cluster and resources, use the following commands:
	Map Hosts: ansible-playbook demo_map_hosts.yml
	Create a standard cluster: ansible-playbook demo_cluster.yml --tags standard
	Create a linked cluster: ansible-playbook demo_cluster.yml --tags linked
	Create a stretched cluster: ansible-playbook demo_cluster.yml --tags stretched
	Remove a cluster: ansible-playbook demo_cluster.yml --tags delete
	Create a resource group: ansible-playbook demo_resource_group.yml --tags create
	Remove a resource group: ansible-playbook demo_resource_group.yml --tags delete
	Add a network: ansible-playbook demo_network.yml --tags create
	Remove a network: ansible-playbook demo_network.yml --tags delete
	Add an interface: ansible-playbook demo_interface.yml --tags create
	Remove an interface: ansible-playbook demo_interface.yml --tags delete
	Add a file system: ansible-playbook demo_file_system.yml --ags create
	Remove a file system: ansible-playbook demo_file_system.yml --tags delete
	Add an application: ansible-playbook demo_applications.yml --tags create
	Remove an application: ansible-playbook demo_applications.yml --tags delete
	Add a volume group: ansible-playbook demo_volume_groups.yml --tags create
	Remove a volume group: ansible-playbook demo_volume_groups.yml --tags delete
	Add a Service IP: ansible-playbook demo_service_ip.yml --tags create
	Remove a Service IP: ansible-playbook demo_service_ip.yml --tags delete
	Start Cluster Services: ansible-playbook demo_start_stop_services.yml --tags start
	Stop Cluster Services: ansible-playbook demo_start_stop_services.yml --tags stop
	Add WPAR: ansible-playbook demo_wpar.yml --tags create
	Remove WPAR: ansible-playbook demo_wpar.yml --tags delete
	Add NFSv2v3: ansible-playbook demo_nfsv2v3.yml --tags create
	Remove NFSv2v3: ansible-playbook demo_nfsv2v3.yml --tags delete
	Add NFSv4: ansible-playbook demo_nfsv4.yml --tags create
	Remove NFSv4: ansible-playbook demo_nfsv4.yml --tags delete
	Install PowerHA: ansible-playbook demo_PowerHA.yml --tags install
	Uninstall PowerHA: ansible-playbook demo_PowerHA.yml --tags uninstall
	Cluster Health: ansible-playbook demo_cluster_health.yml
	Resource Group Move: ansible-playbook demo_move_resource_group.yml 
...
1)	All the field input values are mentioned in the comments in /etc/ansible/external_var.yml
2)	For running start and stop cluster services playbook, we dont need to update any value in /etc/ansible/external_var.yml
3)	NODES and NODE_DETAILS in /etc/ansible/external_var.yml are mandatory for working with any playbook.
4)	You can refer Sample_external_var.yml in /etc/ansible for updating external_var.yml 
